...
export default function CartPage() {
  ...
  const totalPay = productInCart.reduce((acc, { product, quantity }) => acc + product.price * quantity, 0)
        ...
        <div className='flex flex-col w-full sm:w-4/12'>
          <WidgetItem title='Total a pagar'>
            <div className='mt-2 flex justify-center flex-col gap-4'>
              <h3 className='text-3xl font-bold text-gray-700'>Total: ${(totalPay * 1.15).toFixed(2)}</h3>
              <span className='font-bold text-center text-gray-500'>
                {' '}
                Impuestos 15%: ${(totalPay * 0.15).toFixed(2)}
              </span>
              <button className='bg-blue-500 text-white p-2 rounded-lg'>Pagar</button>
            </div>
          </WidgetItem>
        </div>
      </div>
